...
const Home = () => {
  return (
    <div className="min-h-screen bg-slate-50 overflow-hidden font-sans relative">
      ...
      {/* Background moderne avec glow */}
      <div className="absolute top-0 left-0 w-full h-[600px] overflow-hidden -z-10 bg-brand-900 skew-y-[-2deg] origin-top-left shadow-2xl">
        <div className="absolute top-1/2 left-1/4 w-[500px] h-[500px] bg-brand-600 rounded-full blur-[140px] opacity-40 mix-blend-overlay"></div>
        <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-accent-500 rounded-full blur-[120px] opacity-20 mix-blend-overlay"></div>
      </div>

      {/* Navigation (Header) */}
      <header className="absolute top-0 w-full z-10 px-6 sm:px-12 py-6 flex justify-between items-center text-white">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-white rounded flex justify-center items-center shadow-lg font-display text-brand-900 font-bold text-xl">
             N
          </div>
        ...
        <nav className="hidden md:flex gap-8 text-sm font-medium text-blue-100">
          ...
          <Link to="/connexion/interne" className="px-4 py-2 border border-brand-500/50 bg-brand-800/50 hover:bg-brand-700/80 rounded-full backdrop-blur transition-all">
            Espace Conseillers
          </Link>
        </nav>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-12 pt-32 lg:pt-48 pb-20 z-10">
        ...
        <div className="text-center md:text-left max-w-3xl animate-fade-in-up">
          <span className="inline-block px-4 py-1.5 rounded-full bg-brand-800 text-brand-100 font-medium text-xs tracking-wider mb-6 border border-brand-700 uppercase">
          ...
          <h1 className="text-5xl lg:text-7xl font-display font-bold text-white tracking-tight leading-tight mb-6">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-300 to-white">matché aujourd'hui.</span>
          </h1>
          <p className="text-lg lg:text-xl text-brand-100 leading-relaxed max-w-2xl font-light mb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <Link to="/connexion/candidat" className="px-8 py-4 rounded-xl bg-white text-brand-900 font-bold shadow-xl shadow-brand-900/20 hover:scale-105 hover:shadow-2xl transition-all duration-300 text-center">
              Je suis Candidat(e)
            </Link>
            <Link to="/connexion/recruteur" className="px-8 py-4 rounded-xl bg-accent-500 text-white font-bold shadow-lg shadow-accent-500/20 hover:scale-105 hover:bg-accent-600 transition-all duration-300 text-center">
              Je suis Recruteur
            </Link>
          </div>
        </div>
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-6 sm:px-12 -mt-10 lg:-mt-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-8 rounded-2xl shadow-xl hover:shadow-2xl transition hover:-translate-y-2 duration-300 cursor-pointer" onClick={() => window.location.href='/connexion/interne'}>
              <div className="w-12 h-12 bg-brand-100 text-brand-600 rounded-xl flex items-center justify-center text-2xl mb-6">👩‍💼</div>
